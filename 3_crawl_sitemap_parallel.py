...
async def crawl_parallel(urls: List[str], max_concurrent: int =10):
    ...
    dispatcher= MemoryAdaptiveDispatcher(
        memory_threshold_percent=70.0, # dont exceed 70% memory usage
        check_interval=1.0, # checks memory every second
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        # arun_many handles all the urls in parallel, batching and resource management handled by the dispatcher 
        ...
        success_count = 0 
        fail_count = 0 
        # Loop through all crawl results and tally success and failures 
